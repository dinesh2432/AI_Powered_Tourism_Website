...
const RoleBadge = ({ role }) => {
  ...
  return (
    <span className={`text-[8px] font-black uppercase tracking-[0.2em] px-2 py-1 ${styles[role] || styles.viewer}`}>
      {role}
    </span>
  );
};
...
const Avatar = ({ user }) => {
  ...
  return (
    <div className="w-10 h-10 rounded-full bg-primary-500/20 border border-primary-500/30 flex items-center justify-center text-primary-400 font-black text-sm">
      ...
      <div className="card p-10 space-y-10">
          ...
          {isOwner && (
            <div className="flex gap-3">
              <button
                ...
                className="h-10 px-5 bg-white/5 border border-white/10 text-white font-black text-[9px] uppercase tracking-widest hover:bg-white/10 transition-all"
              >
                🔗 Share
              </button>
              <button
                onClick={() => setShowInviteModal(true)}
                className="h-10 px-5 bg-primary-500 text-white font-black text-[9px] uppercase tracking-widest hover:bg-primary-600 transition-all"
              >
                + Invite
              </button>
            </div>
        ...
        {owner && (
          <div className="space-y-3">
            <div className="text-[9px] font-black text-slate-600 uppercase tracking-[0.3em]">Owner</div>
            <motion.div
            ...
            >
              ...
              <div className="flex-1">
                <div className="text-white font-black text-sm uppercase tracking-tight">{owner.name}</div>
                <div className="text-slate-500 text-[9px] font-mono mt-0.5">{owner.email}</div>
              </div>
              ...
              {collaborators.map((c, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-white font-black text-sm uppercase tracking-tight truncate">{c.user.name}</div>
                    <div className="text-slate-500 text-[9px] font-mono mt-0.5 truncate">{c.user.email}</div>
                  </div>
