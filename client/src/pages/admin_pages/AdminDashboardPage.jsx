...
const AdminDashboardPage = () => {
  ...
  const handleApproveApp = (id) => {
    setModal({
      ...
      content: 'Are you sure you want to approve this candidate for Guide Clearance Level 02?',
  ...
  const handleRejectApp = (id) => {
    setModal({
      ...
      content: 'Initiating protocol to decline this application. This action will be logged.',
      confirmText: 'Decline',
      onConfirm: async () => {
        setModal(prev => ({ ...prev, loading: true }));
        try {
  ...
  const handleDeleteVideo = (id) => {
    setModal({
      ...
      content: 'Are you sure you want to permanently delete this media packet from the central database?',
      confirmText: 'Purge',
      onConfirm: async () => {
        setModal(prev => ({ ...prev, loading: true }));
        try {
  ...
  const tabs = ['analytics', 'users', 'applications', 'videos'];
                ...
                {/* <div className="glass-dark border border-white/5 p-8 min-w-[300px]">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex gap-1">
                            {[1,2,3,4,5,6,7,8].map(i => <div key={i} className="w-1.5 h-4 bg-primary-500 shadow-glow-primary animate-pulse" style={{ animationDelay: `${i * 0.1}s` }} />)}
                        </div>
                                    ...
                                    {analytics.recentUsers?.map((u, i) => (
                                        ...
                                        >
                                            <div className="w-12 h-12 rounded-[14px] bg-gradient-to-br from-primary-500 to-purple-600 p-[1px] group-hover:rotate-6 transition-all">
                                                <div className="w-full h-full bg-slate-900 rounded-[13px] flex items-center justify-center text-white font-black">
                                                    {u.name?.charAt(0)}
                                                </div>
                                            </div>
                                    ...
                                    {users.map((u) => (
                                        <tr key={u._id} className="hover:bg-white/[0.02] transition-colors group">
                                            <td className="px-10 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-white font-black text-sm group-hover:rotate-12 transition-all">
                                                        {u.name?.charAt(0)}
                                                    </div>
                                            ...
                                            <td className="px-10 py-6 text-right text-[10px] font-bold text-slate-600 uppercase tracking-widest">
                                        ...
                                        <div className="relative z-10">
                                            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-10">
                                                <div className="flex flex-col md:flex-row md:items-center gap-10">
                                                    <div className="w-24 h-24 rounded-[32px] bg-gradient-to-br from-primary-500 to-purple-600 p-[2px] shadow-glow-primary group-hover:scale-105 transition-transform">
                                                        <div className="w-full h-full bg-slate-900 rounded-[30px] flex items-center justify-center font-black text-3xl text-white">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex flex-wrap gap-3 mb-4">
                                                            ...
                                                            <span className="bg-white/5 text-slate-400 text-[10px] font-black uppercase tracking-widest px-4 py-1 rounded-full border border-white/5">Experience: {app.experience} Years</span>
                                                        </div>
                                                ...
                                                <div className="flex items-center gap-4 xl:shrink-0">
                                                    <button 
                                                        ...
                                                        className="h-16 px-10 rounded-[20px] bg-white text-slate-950 font-black text-xs uppercase tracking-[0.2em] hover:bg-primary-500 hover:text-white transition-all duration-500 shadow-glow-primary hover:shadow-glow-primary"
                                                    >
                                                        Confirm Protocol
                                                    </button>
                                                    <button 
                                                        onClick={() => handleRejectApp(app._id)} 
                                                        className="h-16 px-10 rounded-[20px] glass-dark border border-white/5 text-slate-400 font-black text-xs uppercase tracking-[0.2em] hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/30 transition-all duration-500"
                                                    >
                                                        Decline
                                                    </button>
                                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {videos.map((video) => (
                                    <motion.div 
                                        key={video._id} 
                                        whileHover={{ y: -10 }}
                                        className="glass-dark border border-white/5 rounded-[40px] overflow-hidden group shadow-2xl"
                                    >
                                        <div className="relative h-56 overflow-hidden">
                                            ...
                                            <div className="absolute top-6 left-6 flex gap-2">
                                                <span className="bg-slate-950/80 backdrop-blur-md text-white text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-white/5">
                                                    {video.destination || 'GLOBAL'}
                                                </span>
                                            </div>
                                            <button 
                                                onClick={() => handleDeleteVideo(video._id)}
                                                className="absolute top-6 right-6 w-10 h-10 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-xl flex items-center justify-center backdrop-blur-md border border-red-500/20 transition-all"
                                            >
                                                🗑️
                                            </button>
                                        </div>
